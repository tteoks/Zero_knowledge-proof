...
int main(int argc, char *argv[]) {
  ...
  char infectees_sha[4096][SHA256_DIGEST_LENGTH * 2 + 1] = {
      0x00,
  };
  char client_result[4096][256] = {
      0x00,
  };
  char *temp;
  char select;
  ...
  // SHA256 logic
  printf("### [PASSENSER INFORMATION] Start Hashing...\n");
  while (sql_row = mysql_fetch_row(sql_res)) {
    snprintf(client_result[num], strlen(sql_row[0]) + 1, "%s", sql_row[0]);
    printf("  - %s\n", sql_row[0]);
